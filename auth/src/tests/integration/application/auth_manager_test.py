...
class TestAuthManager(TestConfigCache):
    ...
    def test_user_cache_operations(self):
        self.cacheService.set_user(self.user_john_doe.userId, self.user_john_doe)
        self.cacheService.set_user(self.user_susan.userId, self.user_susan)

        user_cache = self.cacheService.get_user(self.user_john_doe.userId)
        ...
        user_cache = self.cacheService.get_user(self.user_susan.userId)
        self.assertIsNotNone(user_cache)
